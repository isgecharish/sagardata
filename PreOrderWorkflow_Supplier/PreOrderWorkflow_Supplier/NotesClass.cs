...
namespace PreOrderWorkflow_Supplier
{
    public class NotesClass
    {
        ...
        public DataTable GetSupplierNotes()
        {
            return SqlHelper.ExecuteDataset(Con, CommandType.Text, @"Select Notes_RunningNo,NotesId,NN.NotesHandle,IndexValue,Title,Description,
                        NN.UserId, Created_Date, TableDescription,NN.SendEmailTo,
                        (select WF.SupplierName FROM WF1_PreOrder WF WHERE WF.RandomNo=NN.UserId) as SupplierName,
                        (select EMP.EmployeeName FROM HRM_Employees EMP WHERE EMP.CardNo=NN.USerId) as BuyerName
                        from Note_Notes NN
                        INNER JOIN Note_Handle NH ON NH.NotesHandle = NN.NotesHandle
                    Where NN.NotesHandle= '" + NotesHandle + "' and NN.IndexValue= '" + IndexValue+"'").Tables[0];
        }
        ...
        public DataTable GetEmployeeMAilID()
        {
            return SqlHelper.ExecuteDataset(Con, CommandType.Text, @"select WF.UserID, UserFullName, EmailID
                                         from WF1_PreOrder WF
                                         INNER JOIN aspnet_users HE ON HE.LoginId=WF.UserId
                                         where RandomNo='" + User+"'").Tables[0];
        }
